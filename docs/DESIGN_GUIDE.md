...
- **Switch:** one style everywhere — 44×26 px track fully rounded, 22 px white knob in both themes, off `--fill-strong`, on `--live-dot`. On is `--live-dot`, never `--live`: green on a switch is the platform's own idiom, but the day-of token stays on the day.
