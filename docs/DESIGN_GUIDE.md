...
Same resize handles as a card (§4.3: 34×9 hollow `--sel` capsule), astride its own top/bottom edge. On a tall block, hidden until selected or — where there's a mouse — hovered/focused, same as a card's; selecting the block (tapping anything but its + button) is what reveals them, and the + button opens the open-time actions (§5.9 of `FUNCTIONAL_SPEC.md`) instead of selecting. A thin block (under 70 px, no + button and no selected state) shows both always, with a 32 px touch target rather than the usual 44 (a full-size target on both edges would cover the whole block and leave nothing for its own tap). If resizing one side would touch a locked activity, its capsule is drawn with a `--faint` border instead of `--sel` and does nothing when dragged, rather than the handle simply being absent. Whichever activity sits right against a thin block's always-visible handle leaves its own matching handle out entirely — hovering or selecting that card would otherwise draw a second, identical capsule on the exact same edge.
