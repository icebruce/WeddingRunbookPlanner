...
The bar draws no hairline against the live strip: the strip is a tinted band with a rule of its own, and a second rule between the two read as the bar having grown one. `--topbar-height` is floored rather than rounded, so a fractional bar tucks the strip a sliver *under* it instead of leaving a seam the plan scrolls through.
...
- **Alert:** 36 px side margins, 18 px radius, title 17/650, body 14, stacked 46 px buttons, 10 px radius, 15/650. Filled carries the action, quiet carries the way back, and the action is on top. The filled button is `--ink` — the alert is the one place a primary is not `--sel`, because an alert is a decision being forced rather than something offered to tap (D39). Where the action throws something away it is filled `--bad` instead: the same button, the hue changed, never a quieter one. There is no icon well and no footnote; what the alert has to say is the title and one line under it.
