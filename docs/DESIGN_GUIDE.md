...
Day-of: the title is not collapsed into the bar, it simply lives there (D25) — no fade, nothing to trigger, and the left cell is never empty. It is the page's `h1` in that mode. Phone shows the title alone; desktop adds the span below it. Right side: `🔒 View only` pill (30 px) + **Edit** text button (17/600 `--sel`). Editing on the day: amber `● Editing` pill + **Done**.
...
### 4.6 Live strip
Sticky under the top bar, full width, `--live-tint` at 94 % with blur, hairline `rgba(30,158,82,.18)` bottom border, no radius, no shadow. It is the day's header: the large title and the summary line are not shown while it is (D25), and the plan starts 20 px below it.
