...
| `--live` / `--live-dot` / `--live-tint` | `#1E9E52` / `#34C759` / `#EEF7F1` | `#3DD06C` / `#34C759` / `#16301F` | Day-of strip, current activity outline and tag, time line and pill, progress; `--live-dot` also the on-state of every switch (§4.7) |
...
Left: empty at rest — the large title is immediately below, and the planner name above it said the same thing twice — then, once scrolled, the collapsed title (`Wedding Day` 17/650 over `11:30 AM – 10:45 PM` 12, tabular) fades into that space. The two share one grid cell and cross-fade over 160 ms with a 2 px rise; neither is ever `display: none`, so the handover costs no reflow. The line that triggers it is the bottom of everything stuck to the top of the screen — the bar plus the live strip and any pinned bar, not the bar alone — with a 10 px band: collapse as the title goes under it, restore only once it is 10 px clear, so resting the scroll on the handover cannot flip it back and forth. Right: save state (footnote, 7 px dot), then 44 px menu button. Desktop adds status control (34 px pill) before the menu.
Day-of: the title is not collapsed into the bar, it simply lives there (D34) — no fade, nothing to trigger, and the left cell is never empty. It is the page's `h1` in that mode. Phone shows the title alone; desktop adds the span below it. Right side: `🔒 View only` pill (30 px) + **Edit** text button (17/600 `--sel`). Editing on the day: amber `● Editing` pill + **Done**.
The bar draws no hairline against the live strip: the strip is a tinted band with a rule of its own, and a second rule between the two read as the bar having grown one. `--topbar-height` is the bar's exact measured height, fractions and all — a sticky element rests at its natural position until it sticks, so any rounding is a jump the strip makes on the first scroll.
Gutters: 18 px left, 12 px right. Uneven on purpose — the left edge is text, the right edge a 44 px icon button that carries its own optical inset. The bars that stick under it are plain boxes and are evenly guttered instead.
Day-of the bar is opaque `--bg` rather than glass, with the band below it (§4.6).
...
### 4.6 Live panel
A graphite card in a band, sticky under the top bar. The **band** (`.live-strip`) is the page's own tint, opaque and unblurred, full width, 8/12/12 px padding — so the plan scrolls under something rather than behind a floating card, and the bar above draws no rule against it because the two are the same colour (D34). Opaque rather than glass, and the top bar with it on this screen: glass takes its colour from whatever is behind it, and at rest there is nothing behind these two but the page, so the first scroll was the first time the effect had anything to work with and both visibly changed tone as the plan arrived underneath. The **panel** (`.live-panel`) is `--live-panel` #2E323A, 16 px radius, `0 4px 12px rgba(0,0,0,.18)` — a shadow reaches about half its blur beyond the box, and this one has to land on the band's 12 px of padding rather than on the plan scrolling behind it. Same surface in both themes: lighter than the dark page, darker than the light one, so it reads as raised either way (D13).
...
- **Stepper:** 36 px track, 44 px buttons, value 16/600. A reading that explains the value (Duration's `2 hr 10 min`) sits under the row's label as a 13 px `--soft` sub-line, never beside the stepper: a value-width item on the row moves the buttons between taps, and a grouped block is a `<fieldset>`, which will not shrink below its widest row.
- **Switch:** one style everywhere — 44×26 px track fully rounded, 22 px white knob in both themes, off `--fill-strong` under a 1 px inset `--soft` ring, on `--live-dot` with the ring gone. On is `--live-dot`, never `--live`: green on a switch is the platform's own idiom, but the day-of token stays on the day. The off ring is what makes it a control: the track alone is 1.10:1 on the `--surface-2` of a grouped row, and a fill dark enough to carry that edge would outweigh a green on-state at 2.01:1. Outlined while off, filled once on — the map button's bargain.
- **Picker field:** a value you press, not a box you type in — 36 px tall, `--fill-strong`, 9 px radius, 15/550 tabular, with a 16 px glyph before the reading. `--fill-strong`, not `--fill`: every picker sits in a grouped block, and `--fill` on `--surface-2` is under one L* step apart — on a phone there is no hover to reveal it, so the value read as a calculated one. It is the stepper's fill, and the two share a block. An unset field reads its own prompt ("Earliest activity", else `Not set`) at weight 400 in `--ink-2` — the weight tells it from a value, not the colour: `--faint`, which §2 gives placeholders on the page, is 2.62:1 on a `--fill-strong` chip. Never a `readonly` input: that announces as disabled, which is the one thing it is not.
...
- Use `dvh` for sheet heights; sheets scroll their body and keep the header visible; focused fields scroll into view above the keyboard. `dvh` follows the browser's chrome, not the keyboard, so the sheet is lifted by `--keyboard-inset`: the layout viewport's height less the bottom edge of `visualViewport`, which is 0 wherever the platform already shrinks the layout viewport itself and needs no learned baseline. A zoom shrinks the visual viewport the same way a keyboard does, so a reading at any scale but 1 is dropped. The inset follows the viewport alone and never focus: a keyboard outlives the field that opened it, and a sheet that resizes on the tap dismissing it moves the control out from under the finger. Sheets set `touch-action: manipulation`: two taps in a row on a grid of chips is a choice, not a zoom.
