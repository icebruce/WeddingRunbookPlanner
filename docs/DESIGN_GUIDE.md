...
- **Alert:** 36 px side margins, 18 px radius, icon well 44 px, title 17/650, body 14, stacked 46 px buttons (primary filled `--sel`), footnote 13. An alert whose answer throws something away — the delete question — fills that first button `--bad-tint` on `--bad-text` instead, the same pair every other destructive control uses; the way back stays quiet underneath.
