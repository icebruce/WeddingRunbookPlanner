...
Opened by a tall block's + button, or by a long press (touch) / double-click (mouse) on the block itself, thin or not (§3.4). Header: `35 min open before Ceremony · 2:10 – 2:45 PM`. Open time is computed fresh each time from the current gaps between activities — it is never something stored on an activity that these actions "clear".
