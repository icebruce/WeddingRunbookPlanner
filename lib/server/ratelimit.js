...
/**
 * Both counters in one round trip, so a burst of concurrent failures from the
 * same address cannot land two separate INCR/EXPIRE pairs out of step with
 * each other the way two independent `bump()` calls could.
 */
const BUMP_BOTH = `
 ...
 * This assumes Vercel's edge is what sets the header, stripping any value a
 * client sent itself — true in production, but not something this function
 * can verify on its own if the app were ever deployed behind a different or
 * misconfigured proxy.
 */
...
export async function recordLoginFailure(req, { now = Date.now(), client = getUpstash() } = {}) {
  if (!client) {
    bumpMemory(ipKey(req), now);
    bumpMemory(GLOBAL_KEY, now);
    return;
  }
  await client.write(['EVAL', BUMP_BOTH, '2', ipKey(req), GLOBAL_KEY, String(WINDOW_SECONDS)]);
}
