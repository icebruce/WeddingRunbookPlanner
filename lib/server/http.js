 ...
 *
 * `x-forwarded-host`/`x-forwarded-proto` are trusted here, which is only
 * correct because this app is served from behind Vercel's edge, which sets
 * them itself and strips any client-supplied copy. Deployed behind a
 * different or misconfigured proxy that passes a client's own
 * `x-forwarded-*` headers through unchanged, this check could be spoofed.
 */
