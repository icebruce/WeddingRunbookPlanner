...
function normalizeVersion(version) {
  return {
    ...
    // Absent on anything written before routine backups existed, which is
    // what keeps thinning away from them: only a `backup` is ever thinned.
...
/** How long the plan goes without a routine copy being kept. */
export const AUTO_SNAPSHOT_MS = 6 * 60 * 60 * 1000;

export async function savePlan(plan, expectedRevision, { updatedBy = null, now = Date.now() } = {}) {
  ...
  // Whether a backup is due is read off the envelope this save is already
  // writing, not out of the versions list: a save happens every second or so
  // while someone is working, and a second round trip on each of them to ask a
  // question whose answer is "no" some thousands of times running is not a
  // price worth paying for it.
  const due = !current.autoSnapshotAt || (now - new Date(current.autoSnapshotAt).getTime()) >= AUTO_SNAPSHOT_MS;

  const envelope = nextEnvelope(current, {
    plan: validated,
    updatedBy,
    autoSnapshotAt: due ? new Date(now).toISOString() : (current.autoSnapshotAt ?? null)
  });
  ...
  if (due) {
    // A copy of what this save replaced, not of what it wrote. Restoring is
    // for going back, and the state worth having kept is the one that existed
    // before the window's edits — which on the very first save is the plan as
    // it stood before anybody touched it.
    //
    // Written after the plan, and never allowed to fail it. The stamp has
    // moved either way, so a backup that could not be written costs one window
    // rather than being retried on every save until it succeeds.
    await createVersion('Automatic backup', { auto: true, kind: 'backup', plan: current.plan })
      .catch(error => log.error('storage', 'auto_snapshot_failed', error, { revision: envelope.revision }));
  }
...
/**
 * How much history a routine backup is standing in for.
 *
 * Forty slots filled every six hours is ten days, and then the oldest thing
 * anyone can go back to is always about a week ago — which is the wrong week
 * whenever it matters. Keeping them at a resolution that drops with age costs
 * the same forty slots and spans months instead: every one from today, one a
 * day for the past week, one a week for two months, one a month before that.
 *
 * Only routine backups are thinned. The copy written when a conflict was
 * resolved is the losing side of a real decision, and a version someone named
 * is one they meant to keep; neither is a sample of anything.
 */
function bucketOf(version, now) {
  const at = new Date(version.createdAt).getTime();
  if (!Number.isFinite(at)) return null;

  // Bands are measured back from now rather than aligned to the calendar. Two
  // backups three hours apart belong together whether or not a midnight
  // happens to fall between them, and "a day old" should not mean something
  // different at 00:30 than it did at 23:30.
  const age = now - at;
  if (age < DAY_MS) return `h:${version.createdAt}`;
  if (age < 7 * DAY_MS) return `d:${Math.floor(age / DAY_MS)}`;
  if (age < 60 * DAY_MS) return `w:${Math.floor(age / (7 * DAY_MS))}`;
  return `m:${Math.floor(age / (30 * DAY_MS))}`;
}

function thin(versions, now = Date.now()) {
  const seen = new Set();
  return versions.filter(version => {
    if (version.kind !== 'backup') return true;
    const bucket = bucketOf(version, now);
    if (bucket === null) return true;
    // Newest first, so the one kept for each bucket is the most recent in it.
...
export async function createVersion(name, { auto = false, plan = null, kind = null } = {}) {
