...
export function getCookie(req, name = COOKIE_NAME) {
  ...
  for (const part of header.split(';')) {
    ...
    try {
      ...
      // A cookie with a broken percent-escape throws out of decodeURIComponent.
      // Something we did not write put it there, and it is certainly not a
      // token we signed — but letting the URIError out turned every request
      // this browser made into a 500 it could not clear from inside the app.
      return raw;
    }
  }
  return null;
